...
void CConsoleSCB::SetupOptionsMenu()
{
    ...
    connect(clearAction, &QAction::triggered, ui->textEdit, &AzToolsFramework::ConsoleTextEdit::clear);
    m_optionsMenu->addAction(clearAction);
}
...
void CConsoleSCB::RefreshStyle()
{
    ...
    ui->button->setToolTip(tr("Show Console Variables"));
    ui->button->setAccessibleName(tr("Show Console Variables"));

    ui->findButton->setIcon(QIcon(ConsoleConstants::SearchIcon));
    ui->findButton->setToolTip(tr("Toggle Search Bar"));
    ui->findButton->setAccessibleName(tr("Toggle Search Bar"));
