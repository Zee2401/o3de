    ...
    , m_backgroundTheme(gSettings.consoleBackgroundColorTheme)
{
    ...
    ui->lineEditFind->setPlaceholderText(tr("Search..."));
    ...
    ui->lineEdit->setAccessibleName(tr("Console Input"));
...
void CConsoleSCB::SetupOptionsMenu()
{
    ...
    QAction* clearConsoleAction = new QAction(tr("Clear Console"), this);
    connect(clearConsoleAction, &QAction::triggered, [this]() { ui->textEdit->clear(); });
    ...
    , m_itemDelegate(new ConsoleVariableItemDelegate(this))
{
    ...
    // Setup a filter widget with a search line edit input for filtering
