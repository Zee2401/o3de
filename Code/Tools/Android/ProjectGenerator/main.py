...
class TkApp(tk.Tk):
    ...
    def __init__(self, config: ConfigData, config_file_path: str = ""):
        ...
        btn = tk.Button(self, text="Generate Project", command=self.on_generate_project_button, underline=0)
        btn.grid()
    ...
    def _init_load_save_ui(self):
        ...
        btn = tk.Button(apg_settings_frame, text="Load", command=self.on_load_settings_button, underline=0)
        btn.grid(row=row_number, column=2, padx=2, sticky=tk.E)
        self.bind("<Alt-l>", lambda event: self.on_load_settings_button())
        self.bind("<Alt-L>", lambda event: self.on_load_settings_button())

        btn = tk.Button(apg_settings_frame, text="Save", command=self.on_save_settings_button, underline=0)
    ...
    def _init_keystore_settings_ui(self):
        ...
        btn = tk.Button(keystore_frame, text="Create Keystore", command=self.on_create_keystore_button, underline=0)
        btn.grid()
