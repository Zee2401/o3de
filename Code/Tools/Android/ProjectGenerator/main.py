...
class TkApp(tk.Tk):
    ...
    def __init__(self, config: ConfigData, config_file_path: str = ""):
        ...
        self.bind("<Alt-G>", lambda event: self.on_generate_project_button())
        self.bind("<Alt-r>", lambda event: self.clear_log())
        self.bind("<Alt-R>", lambda event: self.clear_log())
    ...
    def _init_report_ui(self):
        ...
        self._report_text_widget.grid(row=0, column=0, sticky=tk.NSEW)
        self._report_scrollbar_widget.grid(row=0, column=1,sticky=tk.NSEW)

        # Clear Log button below the scrollable text widget
        btn_clear = tk.Button(operations_report_frame, text="Clear Log", command=self.clear_log, underline=4)
        btn_clear.grid(row=1, column=0, columnspan=2, pady=2, sticky=tk.E)
        _ToolTip(btn_clear, "Clear operations report log (Alt-R)")
    ...
    def clear_log(self):
        """
        Clears all content from the operations report widget.
        """
        self._report_text_widget.configure(state=tk.NORMAL)
