...
class TkApp(tk.Tk):
    ...
    def _init_report_ui(self):
        ...
        btn_clear_log = tk.Button(operations_report_frame, text="Clear Log", command=self.on_clear_log_button, underline=4)
        btn_clear_log.grid(row=1, column=0, columnspan=2, sticky=tk.E, padx=5, pady=2)
        _ToolTip(btn_clear_log, "Clear all operations log messages (Alt-R)")
    ...
    def on_clear_log_button(self):
        """
        Clears the operations report text area.
        """
        self._report_text_widget.configure(state=tk.NORMAL)
