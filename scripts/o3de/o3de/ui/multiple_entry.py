...
class Dialog(object):
    ...
    def __init__(self, parent, input_value):
        ...
        button_ok = tk.Button(button_frame, text="Ok", width=4, command=self._on_ok, underline=0)
        button_ok.grid(row=0, column=0, sticky=tk.E)

        button_cancel = tk.Button(button_frame, text="Cancel", width=4, command=self._on_cancel, underline=0)
